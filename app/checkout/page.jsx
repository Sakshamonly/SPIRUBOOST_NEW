...
const PaymentSection = ({ total, isSubmitting }) => {
  return (
    <section className="rounded-3xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      {/* Cash on Delivery Option - Blocked */}
      <div className="mt-4 rounded-2xl border border-gray-300 bg-gray-50 p-4 opacity-60">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            <div className="rounded-2xl bg-white p-2.5 text-gray-400 shadow-sm">
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          <span className="shrink-0 rounded-full bg-gray-300 px-3 py-1 text-xs font-semibold text-gray-700">
...
export default function CheckoutPage() {
  ...
  const deliveryFee = useMemo(() => {
    return subtotal < 500 ? 49 : 0
  }, [subtotal])

  const handlingFee = 0
  const total = Math.max(subtotal - discountAmount + deliveryFee + handlingFee, 0)
