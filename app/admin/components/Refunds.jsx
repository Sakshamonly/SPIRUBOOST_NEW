...
const normalizeRefund = (refund) => ({
  ...
  customer: refund?.customer || refund?.user?.name || "Customer",
  amount: Number(refund?.amount ?? 0),
  reason: refund?.reason || "",
  status: refund?.status || "Pending",
  initiatedDate: refund?.initiatedDate
    ? String(refund.initiatedDate).slice(0, 10)
    : refund?.createdAt
    ? String(refund.createdAt).slice(0, 10)
    : "",
  completedDate: refund?.completedDate
    ? String(refund.completedDate).slice(0, 10)
    : null,
});
...
export default function Refunds() {
  ...
  const [newRefundData, setNewRefundData] = useState(emptyRefundData());
  ...
  const fetchData = async () => {
    try {
      ...
      const [refundRes, orderRes] = await Promise.allSettled([
        API.get("/refunds/admin/all"),
        API.get("/orders/admin/all"),
      ]);

      const hasRealFailure = [refundRes, orderRes].some(
        (result) =>
          result.status === "rejected" &&
          result.reason?.response?.status !== 404
      );
      if (hasRealFailure) {
        const firstFailure = [refundRes, orderRes].find(
          (result) =>
            result.status === "rejected" &&
            result.reason?.response?.status !== 404
        );
      ...
      const refundResponse =
        refundRes.status === "fulfilled" ? refundRes.value : null;
      const orderResponse = orderRes.status === "fulfilled" ? orderRes.value : null;

      const refundList = refundResponse
        ? Array.isArray(refundResponse.data)
          ? refundResponse.data
          : refundResponse.data?.refunds || refundResponse.data?.data || []
        : [];

      const orderList = orderResponse
        ? Array.isArray(orderResponse.data)
          ? orderResponse.data
          : orderResponse.data?.orders || orderResponse.data?.data || []
        : [];

      setRefunds(refundList.map(normalizeRefund));
      setOrders(orderList.map(normalizeOrder));
    } catch (err) {
      setError(err?.response?.data?.message || "Failed to load refunds data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const usedRefundOrderIds = useMemo(() => {
    return new Set(refunds.map((refund) => refund.orderId));
  }, [refunds]);

  const failedDeliveryOrders = useMemo(() => {
    return orders.filter(
      (order) =>
        ["Shipped", "Delivery Failed", "Cancelled"].includes(order.status) &&
        !usedRefundOrderIds.has(order.id)
    );
  }, [orders, usedRefundOrderIds]);

  const filteredOrders = useMemo(() => {
    const query = searchOrderInput.toLowerCase().trim();
    return orders.filter((order) => {
      const matchesSearch =
        order.id.toLowerCase().includes(query) ||
        order.customer.toLowerCase().includes(query) ||
        order.email.toLowerCase().includes(query);
      return matchesSearch && !usedRefundOrderIds.has(order.id);
    });
  }, [orders, searchOrderInput, usedRefundOrderIds]);
  ...
  const handleInitiateRefund = async () => {
    ...
    try {
      ...
      setNewRefundData(emptyRefundData());
      setSearchOrderInput("");
      setShowInitiateModal(false);
      await fetchData();
    } catch (err) {
      console.warn("Initiate refund failed");
  ...
  const handleStatusChange = async (refundId, status) => {
    try {
      ...
      console.warn("Update refund status failed");
  ...
  const handleAutoRefund = async (order) => {
    try {
      setActionLoading(true);
      setError("");
      ...
      console.warn("Auto refund failed");
      setError(err?.response?.data?.message || "Failed to create auto-refund");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const inProgressRefunds = refunds.filter((r) => r.status === "In Progress").length;
      ...
      <div className="p-4 sm:p-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              onClick={fetchData}
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors font-medium text-sm"
            >
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-border bg-muted">
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground">
                  Refund ID
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground">
                  Order ID
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground hidden sm:table-cell">
                  Customer
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground">
                  Amount
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground hidden md:table-cell">
                  Reason
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground">
                  Status
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground hidden lg:table-cell">
                  Initiated
                </th>
                <th className="text-left py-3 sm:py-4 px-3 sm:px-6 font-semibold text-foreground">
                  Actions
            ...
            <tbody>
              {loading ? (
                ...
                filteredRefunds.map((refund) => (
                  ...
                  >
                    <td className="py-3 sm:py-4 px-3 sm:px-6 text-foreground font-medium text-xs sm:text-sm">
                      {refund.id}
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6 text-foreground text-xs sm:text-sm">
                      {refund.orderId}
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6 text-foreground hidden sm:table-cell text-xs sm:text-sm">
                      {refund.customer}
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6 text-foreground font-semibold text-xs sm:text-sm">
                      ₹{refund.amount.toLocaleString("en-IN")}
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6 text-foreground text-xs hidden md:table-cell">
                      {refund.reason}
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6">
                      <button
                        ...
                        className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold cursor-pointer hover:opacity-80 transition-opacity ${getStatusColor(
                          refund.status
                        )}`}
                      >
                        {refund.status}
                    ...
                    <td className="py-3 sm:py-4 px-3 sm:px-6 text-foreground hidden lg:table-cell">
                      {refund.initiatedDate}
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6">
                      <button
                        onClick={() => {
        ...
        {failedDeliveryOrders.length > 0 && (
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-6">
            <h2 className="text-lg font-bold text-orange-900 mb-4">
              Orders with Failed Delivery / Eligible Refunds
            </h2>
            <div className="space-y-2">
              {failedDeliveryOrders.map((order) => (
                <div
                  key={order.id}
                  className="flex items-center justify-between p-3 bg-white border border-orange-100 rounded-lg"
                >
                  <div>
            ...
            <div className="bg-white border border-gray-300 rounded-lg p-6 shadow-lg w-full max-w-md max-h-screen overflow-y-auto">
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="Type order ID or customer..."
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewRefundData({ ...newRefundData, orderId: e.target.value })
                    }
                  ...
                  >
                    ...
                    {(searchOrderInput ? filteredOrders : orders.filter((o) => !usedRefundOrderIds.has(o.id))).map(
                      (order) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewRefundData({ ...newRefundData, reason: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400"
                  >
              ...
              <div className="space-y-2 mb-6">
                {refundStatuses.map((status) => (
                  <button
                    key={status}
                    onClick={() => {
                      setNewRefundStatus(status);
                      handleStatusChange(selectedRefund.id, status);
                    }}
